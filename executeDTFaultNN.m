...
load('fault5001.mat')             %actuator fault
% load('fault5010.mat')             %sensor fault
tempFault=[simout(:,3),simout(:,2)];
controlFault=[simout(:,3),simout(:,1)];
referenceData=[ones(length(tempFault),1) [20*ones(20,1)' 50*ones(length(tempFault)-20,1)']'];
...
%% real data vs Digital Twin fault detection
timeFaultVect=faultEvalDataDT(:,5);
faultThresholdDT=faultEvalDataDT(:,1);
faultAlphaDT=faultEvalDataDT(:,2);
faultParityDT=faultEvalDataDT(:,3);
faultNNDT=faultEvalDataDT(:,4);

faultThresholdReal=faultEvalDataReal(:,1);
faultAlphaReal=faultEvalDataReal(:,2);
faultParityReal=faultEvalDataReal(:,3);
faultNNReal=faultEvalDataReal(:,4);


figure()
plot(timeFaultVect,faultThresholdDT)
hold on
plot(timeFaultVect,faultThresholdReal)
legend('DT','Real')
% title('Sensor Fault Thresholding Detector')
title('Actuator Fault Thresholding Detector')
xlabel('Time (s)')
xlim([0 500])
set(gca,'FontSize',14)
...
figure()
plot(timeFaultVect,faultNNDT)
hold on
plot(timeFaultVect,faultNNReal)
legend('DT','Real')
% title('Sensor Fault NN Detector')
title('Actuator Fault NN Detector')
xlabel('Time (s)')
xlim([0 500])
set(gca,'FontSize',14)
